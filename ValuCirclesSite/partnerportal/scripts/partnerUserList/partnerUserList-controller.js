...
'use strict';
  ...
  angular.module('partnerUserList')
  .controller('partnerUserListController',['$state','$http','$scope','getcokkies','config',
  function ($state,$http,$scope,getcokkies,config) { 
    $scope.loader = true;
   /* checking for page loading $scope.config = {
      itemsPerPage: 10,
      fillLastPage: true
    }
    $scope.currentPage = 0;
   ...
   // }).then(function successCallback(response) {
    ...
    $http({
	  ...
	  $scope.data = lpiuserlist.filter(function (a) {
			return a.LoanInitiated == '1' && a.LPiCount > 0 ;
		});
      $scope.loader = false;
	...
	, function errorCallback(response) {
      ...
      // or server returns response with an error status.
						...
						if (response.status != 401) {
							swal("In partneruserlist-controller.js");
